...
table = boto3.resource('dynamodb', region_name=os.environ.get("REGION", "us-east-2")).Table(os.environ.get('TABLE_NAME', 'test'))
...
def getUser(username, table=table):
    resp = table.get_item(Key={'username': username}, AttributesToGet=['username','email']).get('Item')
    return resp

def getUserServices(username, table=table):
    resp = table.get_item(Key={'username': username}, AttributesToGet=['services']).get('Item')
    return resp
...
def user_services_handler(event):
    operation = event['httpMethod']
    if operation == 'PUT':
        return putUserServices(event["pathParameters"]["user"], json.loads(event['body']))
    elif operation == 'GET':
...
def lambda_handler(event, context):
    ...
    if resource_path in resource_paths:
        ...
        return respond(ValueError('Unsupported resource path "{}"'.format(resource_path)))