...
def build_email(user_info, service_info):
    ...
    msg['From'] = "{} <{}>".format('AWS News Service', os.environ.get('SENDER_EMAIL', 'sender@trek10.com'))
    msg['To'] = user_info.get('email')

    msg.preamble = "We found a custom AWS news update based on your saved preferences.\n"
...
def format_html_news(username, raw_user_news):
    ...
    html += "<p>Hello {},<br>Here's a customized AWS product news update based on your saved preferences! You can edit your interests anytime via the What's New API.</p>".format(username)
    for service, items in raw_user_news.items():
        html += "<h2>{}</h2><ul>".format(service)
        for item in items:
            html += '<li><b><a href="{}">{}</a></b></li>'.format(item["links"][0]["href"], item["title"])
